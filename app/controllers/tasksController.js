...
const TaskController = {
  ...
  async updateTask(req, res) {
    ...
    try {
      ...
      console.log('Identifiant du user:', req.session.userId);
      ...
      if (req.session.userId === task.user_id || user.current_coloc_id === creatorUser.current_coloc_id) {
        await Task.update({
          description,
          is_predefined,
          is_done,
  ...
  async deleteTask(req, res) {
    ...
    try {
      ...
      const creatorUser = await Users.findByPk(task.user_id);
      if (req.session.userId === task.user_id || creatorUser.current_coloc_id === user.current_coloc_id ) {
        await Task.destroy({
        where: { tasks_id: taskId }
        });
